...
int drawNCards(int n, int player, struct gameState *state) {
  int i;
  for (i = 0; i < n; i++) {
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  //TODO: Modify this code for assignment 2
  ...
  switch( card ) {
    ...
    case salvager:
      ...
      return salvagerCardEffect(state, choice1, handPos);
    ...
    case treasure_map:
      return treasure_mapCardEffect(state, handPos);
    }
...
int minionCardEffect(struct gameState *state, int choice1, int choice2, int handPos) {
  ...
  //if choice1, +2 coins and return
  if (choice1) {
    state->coins = state->coins + 2;
    return 0;
  }

  //else, current player & other players with hand size > 4 - discard hand and draw 4
  for (i = 0; i < state->numPlayers; i++) {
